...
export default function CameraTestScreen({ navigation }) {
    ...
    (async () => {
      ...
      if (!granted) {
        Alert.alert(
          ...
          'Para este teste, todas as permissões (câmera, áudio, galeria) são necessárias.'
        );
      }
    })();
  }, []);
  ...
  const handleRecordButtonPress = async () => {
    ...
    if (isRecording) {
      // --- Lógica para PARAR ---
      console.log(
        '[TESTE] Botão Parar pressionado. Chamando stopRecording()...'
      );
      ...
      try {
        ...
        Alert.alert(
          'Sucesso!',
          'O vídeo foi gravado. Tentando salvar na galeria...'
        );
        await MediaLibrary.saveToLibraryAsync(data.uri);
        Alert.alert(
          'Salvo!',
          'O vídeo de teste foi salvo na sua galeria com sucesso.'
        );
        navigation.goBack();
      } catch (error) {
        // Tratamento de erro específico e detalhado
        console.error(
          '[TESTE] Erro detalhado durante a gravação:',
        ...
        if (
          ...
          Alert.alert(
            ...
            'O vídeo foi interrompido muito rápido. Tente gravar por mais tempo.'
          );
        } else {
          Alert.alert(
            'Erro de Gravação no Teste',
