...
// MODIFICADO: Apenas as 4 orientações cardeais
const ORIENTATIONS = [
...
// MODIFICADO: Apenas 3 opções de modelo
const MODEL_OPTIONS = [
...
const HomeScreen = ({ navigation, route }) => {
  ...
  const [modelChoice, setModelChoice] = useState('m'); // Padrão agora é 'm' (Normal)
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled">
          <View style={styles.container}>
            ...
            {(appStatus === 'idle' || appStatus === 'selected') && (
               <BigButton title="Como Filmar (Tutorial)" onPress={() => navigation.navigate('OnboardingTutorial')} buttonStyle={styles.tutorialButton}/>
            )}
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  orientationButton: {
    backgroundColor: '#f0f0f0', paddingVertical: 15, borderRadius: 8,
  ...
  cancelSelectionButton: { backgroundColor: '#6c757d', width: '100%', marginTop: 15 },
